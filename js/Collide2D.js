...
/**
 * Hitbox class: a custom hitbox class made by Justin Wise
 * hits is an array of rectangles
 * parent is a reference to the parent object (it must have an x and a y value)
**/

class Hitbox {
	constructor(parent, ...boxes) {
		this.hits = boxes;
		this.parent = parent;
	...
	resetMain() {
		this.mainHit = new Rect(0, 0, 0, 0);
		let left = this.hits[0].left();
		let right = this.hits[0].right();
		let up = this.hits[0].up();
		let down = this.hits[0].down();
		for(var i = 0;i < this.hits.length;i ++) {
			if(left > this.hits[i].left())
				left = this.hits[i].left();
			if(right < this.hits[i].right())
				right = this.hits[i].right();
			if(up > this.hits[i].up())
				up = this.hits[i].up();
			if(down < this.hits[i].down())
				down = this.hits[i].down();
		}
		this.mainHit.x = left/2 + right/2;
		this.mainHit.y = up/2 + down/2;
		this.mainHit.width = right - left;
		this.mainHit.height = down - up;
	}
	
	draw() {
		for(let i = 0;i < this.hits.length;i ++) {
			this.hits[i].draw(this.parent.x, this.parent.y);
		}
	}
	
	broad(hit) {//colliding broadphase
		if(collideRectRect(this.mainHit.x+this.parent.x, this.mainHit.y+this.parent.y, this.mainHit.width, this.mainHit.height, hit.mainHit.x+hit.parent.x, hit.mainHit.y+hit.parent.y, hit.mainHit.width, hit.mainHit.height)) {
			return true;
		} else {
			return false;
		}
	...
	broadDirection(hit) {//colliding broadphase to find direction (Will probably not be used later on
		return directionRectRect(this.mainHit.x+this.parent.x, this.mainHit.y+this.parent.y, this.mainHit.width, this.mainHit.height, hit.mainHit.x+hit.parent.x, hit.mainHit.y+hit.parent.y, hit.mainHit.width, hit.mainHit.height);
	}
	...
	x(i) {
		return this.hits[i].x + this.parent.x;//left
	}
	
	y(i) {
		return this.hits[i].y + this.parent.y;//up
	}
}
...
class Rect {
	...
	draw(x, y) {
		rect(x + this.x, y + this.y, this.width, this.height);
	}
}

p5.prototype.collideHitHit = function(hitbox1, hitbox2) {
	for(let i = 0;i < hitbox1.hits.length;i ++) {
		for(let u = 0;u < hitbox2.hits.length;u ++) {
			if(collideRectRect(hitbox1.x(i), hitbox1.y(i), hitbox1.hits[i].width, hitbox1.hits[i].height, hitbox2.x(u), hitbox2.y(u), hitbox2.hits[u].width, hitbox2.hits[u].height))
				return true;
		}
	}
	return false;
}
p5.prototype.collideHitRect = function(hitbox1, x2, y2, w2, h2) {
	for(let i = 0;i < hitbox1.hits.length;i ++) {
		if(collideRectRect(hitbox1.x(i), hitbox1.y(i), hitbox1.hits[i].width, hitbox1.hits[i].height, x2, y2, w2, h2))
			return true;
	}
	return false;
}

p5.prototype.directionRectRect = function (x, y, w, h, x2, y2, w2, h2) {
	let player_bottom = y + h/2;
	let tiles_bottom = y2 + h2/2;
	let player_right = x + w/2;
	let tiles_right = x2 + w2/2;
	
	let b_collision = tiles_bottom - (y - h/2);
	let t_collision = player_bottom - (y2 - h2/2);
	let l_collision = player_right - (x2 - w2/2);
	let r_collision = tiles_right - (x - w/2);
...
p5.prototype.collideRectRect = function (x, y, w, h, x2, y2, w2, h2) {
  ...
  if (x + w/2 >= x2 - w2/2 &&    // r1 right edge past r2 left
      x - w/2 <= x2 + w2/2 &&    // r1 left edge past r2 right
      y + h/2 >= y2 - h2/2 &&    // r1 top edge past r2 bottom
      y - h/2 <= y2 + h2/2) {    // r1 bottom edge past r2 top
        return true;
  }
  return false;
};
...
p5.prototype.collideRectCircle = function (rx, ry, rw, rh, cx, cy, diameter) {
  ...
  if (cx < rx-rw/2){         testX = rx-rw/2;       // left edge
  }else if (cx > rx+rw/2){ testX = rx+rw/2;  }   // right edge

  if (cy < ry-rh/2){         testY = ry-rh/2;       // top edge
  }else if (cy > ry+rh){ testY = ry+rh/2; }   // bottom edge
...
//2d
if (pointX >= x - xW/2 &&         // right of the left edge AND
    pointX <= x + xW/2 &&    // left of the right edge AND
    pointY >= y - yW/2 &&         // below the top AND
    pointY <= y + yW/2) {    // above the bottom
        return true;
}
return false;
};
